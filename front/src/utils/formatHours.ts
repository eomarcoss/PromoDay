const DIAS_SEMANA: { [key: string]: { nome: string; abrev: string; ordem: number } } = {
  ...
  export function formatBusinessHours(businessHoursData: any): string {
    ...
    const openDays = Object.entries(hours)
      .filter(([_, value]: [string, any]) => value?.aberto === true && value?.inicio && value?.fim)
    ...
    const sameHoursAllDays = openDays.every((d) => `${d.inicio} às ${d.fim}` === firstHours);
